...
float calcularV(float K);
float *reservar(int puntos);
void resolver(float *x, float *y, float *z, float *vx, float *vy, float *vz);
float dvx(float x, float y, float z, float vx, float vy, float vz);
float dvy(float x, float y, float z, float vx, float vy, float vz);
float dvz(float x, float y, float z, float vx, float vy, float vz);
void guardar(float *x, float *y,float *z);
...
main(int argc, char **argv){
  ...
  float k=atof(argv[1])*1.60218*pow(10,-13);
  v=calcularV(k);
  printf("la velocidad es: %f\n",v);
  g=1.0/sqrt(1-v*v/(c*c));
  printf("g es: %f\n",g);
  float alfa=atof(argv[2])*pi/180;
...
float *reservar(int puntos){
  float *array=malloc(puntos*sizeof(float));
  int i;
    for(i=0;i<puntos;i++){
	array[i]=0.0;
      }
...
void resolver(float *x, float *y, float *z, float *vx, float *vy, float *vz){
  int i;
  float xk1,xk2,xk3,xk4,yk1,yk2,yk3,yk4,zk1,zk2,zk3,zk4,
